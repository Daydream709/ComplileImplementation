...
Result Color::Coloring() {
  const int K = registers_->GetList().size();
  ...
  while (!remaining.empty()) {
    ...
    if (chosen == remaining.end()) {
      chosen = std::max_element(remaining.begin(), remaining.end(),
                                [&](live::INodePtr a, live::INodePtr b) {
  ...
  for (auto it = select_stack.rbegin(); it != select_stack.rend(); ++it) {
    ...
    for (auto reg : registers_->GetList()) {
      std::string name = *initial_->Look(reg);
      if (!forbidden.count(name)) {
        assigned[node] = name;
        coloring->Enter(node->NodeInfo(), new std::string(name));
        colored = true;
        break;
      }
    }
