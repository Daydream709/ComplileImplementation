...
void FlowGraphFactory::AssemFlowGraph() {
  ...
  for (std::size_t i = 0; i < nodes.size(); ++i) {
    ...
    if (auto oper = dynamic_cast<assem::OperInstr *>(instr); oper && oper->jumps_) {
      has_jump = true;
      unconditional = oper->assem_.rfind("jmp", 0) == 0;
    ...
    if (i + 1 < nodes.size() && (!has_jump || !unconditional))
...
temp::TempList *MoveInstr::Def() const { return dst_ ? dst_ : new temp::TempList(); }

temp::TempList *OperInstr::Def() const { return dst_ ? dst_ : new temp::TempList(); }

temp::TempList *LabelInstr::Use() const { return new temp::TempList(); }

temp::TempList *MoveInstr::Use() const { return src_ ? src_ : new temp::TempList(); }

temp::TempList *OperInstr::Use() const { return src_ ? src_ : new temp::TempList(); }
