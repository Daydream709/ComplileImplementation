...
type::Ty *SubscriptVar::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  type::Ty *exp_ty = subscript_->SemAnalyze(venv, tenv, labelcount, errormsg)->ActualTy();
...
type::Ty *CallExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  while (formal_it != fun_entry->formals_->GetList().end() &&
    ...
    type::Ty *arg_ty = (*arg_it)->SemAnalyze(venv, tenv, labelcount, errormsg)->ActualTy();
...
type::Ty *RecordExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  while (expected_it != record_ty->fields_->GetList().end() && actual_it != fields_->GetList().end()) {
    if ((*expected_it)->name_ != (*actual_it)->name_) {
      errormsg->Error(pos_, "field %s doesn't exist", (*actual_it)->name_->Name().data());
      return type::IntTy::Instance();
    }
    type::Ty *expected_ty = (*expected_it)->ty_->ActualTy();
    type::Ty *actual_ty = (*actual_it)->exp_->SemAnalyze(venv, tenv, labelcount, errormsg)->ActualTy();
...
void VarDec::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv, int labelcount,
                        err::ErrorMsg *errormsg) const {
  type::Ty *exp_ty = init_->SemAnalyze(venv, tenv, labelcount, errormsg)->ActualTy();
  if (typ_) {
...
void TypeDec::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv, int labelcount,
  ...
  for (NameAndTy *name_and_ty : types_->GetList()) {
    ...
    while (typeid(*cur) == typeid(type::NameTy)) {
      type::NameTy *name_ty = static_cast<type::NameTy *>(cur);
      cur = name_ty->ty_;
      if (cur == ty) {
